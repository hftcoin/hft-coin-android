...
public class TokensRealmSource implements TokenLocalSource {
    ...
    public Token fetchToken(int chainId, Wallet wallet, String address)
    {
        try (Realm realm = realmManager.getRealmInstance(wallet))
        {
            ...
            if (t == null && address.equalsIgnoreCase(wallet.address))
            {
                ...
                // Modified by HFT
                if (info != null) { t = (HFTC_ID == chainId) ? createHFTCToken(wallet) : createCurrencyToken(info, wallet); }
            }
        ...
        try (Realm realm = realmManager.getRealmInstance(walletAddress))
        {
            realm.executeTransaction(r -> {
                ...
                for (NetworkInfo info : allMainNetworks)
                {
                    ...
                    {
                        // Added by HFT
                        if (HFTC_ID == info.chainId)
                        {
                            Wallet wallet = new Wallet(walletAddress);
                            Token token = createHFTCToken(wallet);
                            saveToken(r, token);
                            setEnable(wallet, token, true);
    ...
    // Added by HFT
    private Token createHFTCToken(Wallet wallet)
    {
        TokenInfo tokenInfo = new TokenInfo(C.HFTC_TOKEN, C.HFTC_NAME, C.HFTC_SYMBOL, C.HFTC_DECIMALS, true, HFTC_ID);
        BigDecimal balance = BigDecimal.ZERO;
        Token hftc = new Token(tokenInfo, balance, 0, C.HFTC_SHORT_NETWORK_NAME, ContractType.ERC20); //create with zero time index to ensure it's updated immediately
        hftc.setTokenWallet(wallet.address);
        hftc.pendingBalance = balance;
        return hftc;
    }
}
