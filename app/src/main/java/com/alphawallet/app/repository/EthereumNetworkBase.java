...
import static com.alphawallet.ethereum.EthereumNetworkBase.HFTC_ID; // Added by HFT
...
public abstract class EthereumNetworkBase implements EthereumNetworkRepositoryType
{
    ...
    public static final String HFTC_RPC_URL = "https://hftcoin.org/mainnet-api"; // Added by HFT
    public static final String HFTC_FALLBACK_RPC_URL = HFTC_RPC_URL; // Added by HFT
    public static final String HFTC_TX_URL = "https://hftcoin.org/tx-api"; // Added by HFT
    ...
    //This optional list creates a defined order in which tokens are displayed (Modified by HFT)
    ...
    static final Map<Integer, NetworkInfo> networkMap = new HashMap<Integer, NetworkInfo>() {
        {
            // Added by HFT
            put(HFTC_ID, new NetworkInfo(C.HFTC_NETWORK_NAME, C.HFTC_SHORT_NETWORK_NAME, HFTC_RPC_URL,
                    HFTC_RPC_URL, HFTC_ID, HFTC_FALLBACK_RPC_URL, HFTC_TX_URL));
    ...
    // Control "Select Active Networks"
    @Override
    public Integer getDefaultNetwork(boolean isMainNet)
    {
		return isMainNet ? HFTC_ID : RINKEBY_ID; // Modified by HFT
    }
    ...
    {
        return HFTC_ID == chainId ? true : false; // Modified by HFT
    }

    public static BigInteger gasOverrideValue(int chainId)
    {
        return BigInteger.valueOf(0); // Modified by HFT
    }
    ...
    {
        return new ContractLocator("", HFTC_ID, ContractType.ERC20); // Modified by HFT
    }
