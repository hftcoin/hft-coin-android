...
public class TransactionHolder extends BinderViewHolder<TransactionMeta> implements View.OnClickListener
{
    ...
    public static final int TRANSACTION_BALANCE_PRECISION = C.HFTC_DECIMALS; // Modified by HFT
    ...
    public void bind(@Nullable TransactionMeta data, @NonNull Bundle addition)
    {
        ...
        // Added by HFT
        if (!token.getIsSent(transaction))
        {
            value.setTextColor(getContext().getColor(R.color.hftc));
        }
        else
        {
            value.setTextColor(getContext().getColor(R.color.black));
        }

        // Modified by HFT
        // CharSequence typeValue = Utils.createFormattedValue(getContext(), operationName, shouldShowToken ? token : null);
        CharSequence typeValue = Utils.createFormattedValue(getContext(), operationName, null);
    ...
    {
        if (transaction.chainId == MAINNET_ID || transaction.chainId == HFTC_ID)
        {
    ...
    {
        findViewById(R.id.separator_line).setVisibility(View.GONE);
    }
}
