...
public class NewSettingsFragment extends BaseFragment {
    ...
    private SettingsItemView buyHFTCSetting; // Added by HFT
...
//                        .build();
        ...
        buyHFTCSetting =
                new SettingsItemView.Builder(getContext())
                        .withIcon(R.mipmap.ic_alpha)
                        .withTitle(R.string.title_buy_hftc)
                        .withListener(this::onBuyHFTCSettingClicked)
                        .build();
        supportSetting =
                new SettingsItemView.Builder(getContext())
    ...
    private void addSettingsToLayout() {
        ...
        // Added by HFT
        nameThisWallet.findViewById(R.id.bottom_separator_line).setVisibility(View.GONE);
        ...
        // Added by HFT
        advancedSetting.findViewById(R.id.bottom_separator_line).setVisibility(View.GONE);

        supportSettingsLayout.addView(buyHFTCSetting, supportIndex++); // Added by HFT
        supportSettingsLayout.addView(supportSetting, supportIndex++);
        // Added by HFT
        supportSetting.findViewById(R.id.bottom_separator_line).setVisibility(View.GONE);
    }
    ...
    void addBackupNotice(GenericWalletInteract.BackupLevel walletValue) {
        ...
        // warningSeparator.setVisibility(View.VISIBLE); // Deleted by HFT
        if (wallet != null) {
            backupButton.setText(getString(R.string.action_back_up_my_wallet)); // Modified by HFT
            backupButton.setOnClickListener(v -> openBackupActivity(wallet));
            backupTitle.setText(getString(R.string.wallet_not_backed_up));
            // Modified by HFT
            layoutBackup.setBackgroundResource(R.drawable.background_warning_hftc_inactive_8dp);
    ...
    // Added by HFT
    private void onBuyHFTCSettingClicked() {
        Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(C.BUY_HFTC_URL));
        startActivity(intent);
    }
