        ...
        FragmentMessenger, Runnable, SignAuthenticationCallback, LifecycleObserver
{
    ...
    public HomeActivity()
    {
        ...
        // Modified by HFT
        if (CustomViewSettings.hideActivityFragment()) activityFragment = new Fragment();
        else activityFragment = new ActivityFragment();
    ...
    private void onWalletName(String name)
    {
        ...
        {
            walletTitle = name.trim(); // Modified by HFT
        }
        else
        {
    ...
    private void checkWarnings()
    {
        if (updatePrompt)
        {
            ...
            if (warns < 3)
            {
                ...
                // Modified by HFT
                cDialog.setSmallText(String.format("Using an old version of HFTCoin. Please update from %s.", C.HFTC_WEBSITE));
