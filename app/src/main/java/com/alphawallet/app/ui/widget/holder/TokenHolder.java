...
public class TokenHolder extends BinderViewHolder<TokenCardMeta> implements View.OnClickListener, View.OnLongClickListener {
    ...
    private final ImageView statueOfLiberty; // Added by HFT
    ...
    public TokenHolder(ViewGroup parent, AssetDefinitionService assetService, TokensService tSvs, Realm r)
    {
        ...
        statueOfLiberty = findViewById(R.id.statue_of_liberty); // Added by HFT
    ...
    private void initEvent()
    {
        itemView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                statueOfLiberty.setBackgroundResource(R.drawable.statue_of_liberty_light);
        ...
        itemView.setOnTouchListener(new View.OnTouchListener() {
            final float scaleRate = 0.95f;
            final long durationMillis = 300;
            ...
            public boolean onTouch(View v, MotionEvent event) {
                switch (event.getAction())
                {
                    case MotionEvent.ACTION_DOWN:
                        itemView.animate().scaleX(scaleRate).scaleY(scaleRate).setDuration(durationMillis).start();
                        break;
                    case MotionEvent.ACTION_UP:
                    case MotionEvent.ACTION_CANCEL:
                        itemView.animate().scaleX(1).scaleY(1).setDuration(durationMillis).start();
                        statueOfLiberty.setBackgroundResource(R.drawable.statue_of_liberty);
                        break;
                }
    ...
    public void bind(@Nullable TokenCardMeta data, @NonNull Bundle addition)
    {
        try
        {
            ...
            // Modified by HFT
            tokenLayout.setBackgroundResource(R.drawable.background_token);
            ...
            String nameValue = token.getStringBalance() + " " + token.getSymbol();
            ...
            // Added by HFT
            walletAddress.setText(Utils.formatAddressExt(token.getWallet()));
    ...
    private void populateTicker()
    {
        ...
        // Modified by HFT
        if (!token.isEthereum() && token.tokenInfo.chainId != MAINNET_ID && token.tokenInfo.chainId != HFTC_ID)
        {
            showNetworkLabel();
        }
        else
    ...
    private void setIssuerDetails()
    {
        ...
        else
        {
            ...
            {
                if (issuerName == C.HFTC_SYMBOL) issuerName = C.HFTC_NAME; // Added by HFT
