...
public class BalanceUtils
{
    ...
    private static final int showDecimalPlaces = C.HFTC_DECIMALS; // Modified by HFT
    ...
    // Remove the trailing zeros. Added by HFT
    public static String stripTrailingZeros(String valueStr)
    {
        try
        {
            String valueStrNew = valueStr.replaceAll(",", "");
            BigDecimal value = new BigDecimal(valueStrNew);

            return getValueWithThousandSeparator(value.stripTrailingZeros(), 0);
        }
        catch (Exception e)
        {
            return valueStr;
    ...
    // Added by HFT
    public static String getValueWithThousandSeparator(BigDecimal value, long decimals)
    {
        String valueStr = getScaledValueScientific(value, decimals);
        if (valueStr.equals("0") && value.compareTo(BigDecimal.ZERO) > 0)
        {
            valueStr = "~0";
        }
