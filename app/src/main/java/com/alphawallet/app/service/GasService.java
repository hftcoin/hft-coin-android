...
public class GasService implements ContractGasProvider
{
    ...
    public GasService(EthereumNetworkRepositoryType networkRepository, OkHttpClient httpClient, RealmManager realm)
    {
        ...
        currentChainId = HFTC_ID; // Modified by HFT
    ...
    public void updateChainId(int chainId)
    {
        ...
        {
            currentChainId = chainId;
            // web3j = getWeb3jService(chainId); // Deleted by HFT, hasGasOverride never reaches here.
        }
    ...
    {
        // Added by HFT
        if (HFTC_ID == currentChainId)
        {
            return BigInteger.ZERO;
        }
        else if (gasPrice.compareTo(BigInteger.ZERO) > 0)
        {
            return gasPrice;
        }
        else
