...
import com.alphawallet.app.entity.HFTCTransaction;
...
{
    private final String TAG = "TransactionsNetworkClient";
    ...
    private HFTCTransaction[] getHFTCTransactions(String response) throws JSONException
    {
        JSONObject stateData = new JSONObject(response);
        String error = stateData.getString("error");
        if ("null" != error)
        {
            return null;
        }

        int total = stateData.getJSONObject("result").getInt("total");
        if (0 == total)
        {
            return null;
        }

        JSONArray transactions = stateData.getJSONObject("result").getJSONArray("transactions");
        return gson.fromJson(transactions.toString(), HFTCTransaction[].class);
    }
    ...
    {
        if (BuildConfig.DEBUG) Log.d(TAG, "readTransactions");
    ...
    public Single<Integer> readTransfers(String walletAddress, NetworkInfo networkInfo, TokensService svs, boolean isNFTCheck)
    {
        ...
        if (nftCheck && svs.openSeaUpdateInProgress(networkInfo.chainId)) { return Single.fromCallable(() -> 0); } //don't allow simultaneous NFT checking
            ...
            try (Realm instance = realmManager.getRealmInstance(new Wallet(walletAddress)))
            {
                ...
                EtherscanEvent[] events = readNextTxBatch(walletAddress, networkInfo, lastBlockChecked, nftCheck ? ERC721_QUERY : ERC20_QUERY);
                ...
                if (events != null && events.length > 0)
                {
                    ...
                    //and update the top block read, Modified by HFT
                    lastBlockChecked = Long.parseLong(events[0].blockNumber);
    ...
    // Modified by HFT
    private EtherscanEvent[] readNextTxBatch(String walletAddress, NetworkInfo networkInfo, long currentBlock, String queryType)
    {
        if (BuildConfig.DEBUG) Log.d(TAG, "readNextTxBatch start");
        ...
        okhttp3.Response response;
        String result = "0";
        HFTCTransaction[] txs = null;
        if (currentBlock == 0) currentBlock = 1;

        String jsonBody = String.format("{\"jsonrpc\":\"2.0\",\"method\":\"reporting.GetTransactions\"," +
                "\"params\": [{\"contract\":\"%s\",\"holder\":\"%s\",\"options\":{\"beginBlockNumber\":%d}}]," +
                "\"id\":1}", C.HFTC_TOKEN, walletAddress, currentBlock);
        RequestBody requestBody = FormBody.create(jsonBody, MediaType.parse("application/json"));

        if (BuildConfig.DEBUG) Log.d(TAG, String.format("wallet:[%s], beginBlockNumber:[%d]", walletAddress, currentBlock));

        try
        {
            Request request = new Request.Builder()
                    .url(networkInfo.etherscanTxUrl)
                    .post(requestBody)
                    .build();

            response = httpClient.newCall(request).execute();
            result = response.body().string();
            txs = getHFTCTransactions(result);

            if (txs != null) {
                for (HFTCTransaction tx : txs) {
                    tx.contractAddress = C.HFTC_TOKEN;
                    tx.tokenID = String.valueOf(HFTC_ID);
                    tx.tokenName = C.HFTC_NAME;
                    tx.tokenSymbol = C.HFTC_SYMBOL;
                    tx.tokenDecimal = String.valueOf(C.HFTC_DECIMALS);
                }
            }
        }
        catch (InterruptedIOException e)
