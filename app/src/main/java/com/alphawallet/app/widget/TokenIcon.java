...
public class TokenIcon extends ConstraintLayout
{
    ...
    private void setupDefaultIcon(boolean loadFailed)
    {
        ...
        // Modified by HFT
        if (token.isHFTC() || token.isEthereum() || EthereumNetworkRepository.getChainOverrideAddress(token.tokenInfo.chainId).equalsIgnoreCase(token.getAddress()))
        {
            textIcon.setVisibility(View.GONE);
            icon.setImageResource(EthereumNetworkRepository.getChainLogo(token.tokenInfo.chainId));
            icon.setVisibility(View.VISIBLE);
            // findViewById(R.id.circle).setVisibility(View.VISIBLE); // Modified by HFT
        }
        else
        {
    ...
    public void setStatusIcon(StatusType type)
    {
        ...
        switch (type)
        {
            ...
            case REJECTED:
                ...
                break;
            case CONSTRUCTOR:
                statusIcon.setVisibility(View.GONE); // Modified by HFT
                break;
            case SELF:
    ...
    {
        // Modified by HFT
        if (loadFailed && !token.isHFTC()) icon.setVisibility(View.GONE);
